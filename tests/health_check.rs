...
async fn spawn_app() -> TestApp {
    ...
    let failed_configuration = "Failed to read configuration";
    let mut configuration = get_configuration().expect(failed_configuration);
...
pub async fn configure_database(config: &DatabaseSettings) -> PgPool {
    // Create database
    let mut connection =
        PgConnection::connect(&config.connection_string_without_bd())
            .await
